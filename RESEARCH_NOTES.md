...
| **组合叠加** | 冠军 + ts_corr + 市值 + OBV | ✅ 已验证 | 四合一score=87.5,IR=1.03；五合一score=87.0,IC=0.148；行业相对五合一(hp=21)score=87.5,IR=1.30,MaxDD=-7.1%；**行业相对五合一(hp=5周频)全场最强，anti_overfit 4/4+WF确认** | — | ⚠️ 以上Sharpe/MaxDD多数仍是`ls_returns`理论多空数字；仅"行业相对五合一(hp=5)"已用`strategy_returns`真实多头验证：CAGR超额≈4.47%,Sharpe=0.44,并非每年跑赢。其余候选**待用真实多头重新验证**(见下方TODO) |
...
| | amtrev_x_turn_v2(冠军) | 四合一 | 行业相对五合一(hp=21) | 行业相对五合一(hp=5) |
|---|---|---|---|---|
| 表达式 | `(-1*rank(vol/ts_mean(vol,60)))+rank(-1*ts_mean(vol/total_share,60))` | 冠军+ts_corr+市值 | 冠军+ts_corr+市值+OBV的group_rank版 | 同左，hp=5 |
| IC | 0.084 | 0.137 | 0.120 | 0.090 |
| mono | 0.90 | 1.0 | 1.0 | 1.0 |
| anti_overfit | 4/4 | 4/4 | 4/4 | 4/4(yearly IC逐年单调递增) |
| 真实多头验证 | 未测 | **未测(TODO)** | **未测(TODO)** | ✅ CAGR超额4.47%,Sharpe=0.44 |
| 跨宇宙 | hs300失效 | 未测 | csi500/csi2000通过,hs300未测 | 未测 |
| 推荐用途 | 已知基线 | 待定 | 待定 | 目前唯一验证过真实收益的候选 |

**核心结论未变但需要补验证**：行业相对五合一(hp=5)是唯一经过"真实多头收益"（而非理论多空）验证的候选，其余排名靠前的组合都还只有理论多空数字，需要重新验证才能确定最终排名。
...
- [ ] **用真实多头(strategy_returns)重新验证四合一/五合一/行业相对五合一(hp=21)/ts_corr单独**——目前只有hp=5版本验证过，其余的Sharpe/MaxDD都是理论多空(ls_returns)数字，可能和hp=5版本一样被大幅高估
- [ ] **用刷新后的total_share数据（覆盖到2026-04）重新跑WF滚动验证**——之前受限于旧数据只能测出1-2个窗口，现在应该能多测出至少一个窗口
- [ ] **行业相对五合一(hp=21/hp=5)补充hs300/csi500/csi2000跨宇宙验证**——目前只有原始的冠军和四合一/五合一测过hs300失效，行业相对版本的跨宇宙表现还不确定
